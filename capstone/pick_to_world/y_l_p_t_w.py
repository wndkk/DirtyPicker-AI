...
def run():
    if not os.path.exists(H_PATH):
        raise FileNotFoundError(f"Homography not found: {H_PATH} (run build_homography first)")
    ...
    while True:
        ...
        # draw selected (locked/candidate)
        if sel is not None:
            x1,y1,x2,y2 = sel.xyxy.astype(int)
            cv2.rectangle(frame, (x1,y1), (x2,y2), (0,0,255), 3)

            # pick point + world
            u, v = pick_point_from_box(sel.xyxy, 0.15)
            cv2.circle(frame, (int(u), int(v)), 6, (0,0,255), -1)

            Xw, Yw = uv_to_world(H, u, v)
            cv2.putText(frame, f"Pick UV=({u:.0f},{v:.0f})", (10, 110),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
            cv2.putText(frame, f"World XY=({Xw:.1f},{Yw:.1f}) mm", (10, 140),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
        ...
        cv2.imshow("YOLO LOCK -> WORLD", frame)
