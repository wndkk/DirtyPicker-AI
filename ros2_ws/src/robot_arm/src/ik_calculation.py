...
import math
import timegit config --global user.name
...
class IkCalc(Node):
    def __init__(self):
        ...
        # Task Planner로부터 수신
        self.target_sub = self.create_subscription(Point, '/ik_target_pose', self.target_callback, 10)
        self.gripper_sub = self.create_subscription(Bool, '/ik_gripper_cmd', self.gripper_callback, 10)
