...
class IkCalc(Node):
    ...
    def run(self):
        while(rclpy.ok()):
            x, y, z = input("목적지 x, y, z 좌표와 pitch 각도 (예: 0.1 0.1 0.2) : ").split()
            print(f"입력된 좌표: {x}, {y}, {z}")
            ik_result = self.auto_pitch_calc(float(x), float(y), float(z))
            
            if ik_result is not None:
                
                
                print(f"계산된 관절 각도 (rad): {ik_result}")
    ...
    def ik_calculation(self, x, y, z, pitch_deg):
        ...
        if abs(cos_theta2) >= 1:
            self.get_logger().error('목적지에 도달할 수 없습니다. 범위를 벗어난 위치입니다.')
            return None
...
def main(args=None):
    ...
    node = IkCalc()
    rclpy.spin_once(node) # 응답을 받을 때까지 대기
    node.destroy_node()
    rclpy.shutdown()
